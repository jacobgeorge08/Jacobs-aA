...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  bakePrint(pieQuantity, recipe, pieType)

  printRevenue(pieQuantity, recipe, profitMargin)
}


function bakePrint(pieQuantity, recipe, pieType) {
...
function totalC(pieQuantity, recipe) {
  const totalCost = pieCost(recipe) * pieQuantity;
  return totalCost
}

// Print the total revenue calculated using the given profitMargin
function printRevenue(pieQuantity, recipe, profitMargin) {
  const revenue = totalC(pieQuantity, recipe) * (profitMargin || 1.2);
